...
content = re.sub(
    r'\{showDot && \(\s*<div className="w-2 h-2 rounded-full bg-red-500 shadow-\[0_0_8px_rgba\(239,68,68,0\.8\)\] animate-pulse" />\s*\)\}',
    r'''{unreadCountForNav > 0 ? (
...
content = re.sub(
    r'\{\(item\.href === "#notifications" && unreadCount > 0\) \|\| \(item\.href\.includes\("chat"\) && chatUnread > 0\) && \(\s*<span className="absolute -top-1 -right-1 w-2\.5 h-2\.5 bg-\[#EF4444\] rounded-full animate-pulse border-2 border-\[#07111F\]" />\s*\)\}',
    r'''{((item.href === "#notifications" && unreadCount > 0) || (item.href.includes("chat") && chatUnread > 0)) ? (
